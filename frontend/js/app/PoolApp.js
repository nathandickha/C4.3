...
const STARTER_POOL_PRESETS = [
  {
    ...
    title: "6 x 4 Curved Corner Pool",
    description: "6 x 4 rectangle with a 2 m curved corner, left radius steps, bench seat, and Arctic Blue tile.",
    preview: "rectangle",
    params: {
      shape: "rectangular",
      length: 6,
    ...
    customOutline: { type: "rounded-rectangle-corner", corner: "front-left", radius: 2 },
    tile: "Arctic Blue",
    spa: null
  },
  {
...
updateGroundVoid(this.ground || this.scene.userData.ground, this.poolGroup, this.spa);
  ...
  createStarterRoundedCornerPolygon(config = {}) {
    const length = Math.max(0.1, Number(this.poolParams?.length) || 6);
    const width = Math.max(0.1, Number(this.poolParams?.width) || 4);
    const hx = length * 0.5;
    const hy = width * 0.5;
    const maxRadius = Math.max(0.05, Math.min(length, width) * 0.5 - 0.02);
    const radius = THREE.MathUtils.clamp(Number(config.radius) || 1, 0.05, maxRadius);
    const corner = config.corner || "front-left";

    let vertices;
    let curvedEdgeIndex = 0;
    let control;

    if (corner === "front-left") {
      vertices = [
        new THREE.Vector2(-hx, -hy + radius),
        new THREE.Vector2(-hx + radius, -hy),
        new THREE.Vector2(hx, -hy),
        new THREE.Vector2(hx, hy),
        new THREE.Vector2(-hx, hy)
      ];
      curvedEdgeIndex = 0;
      control = new THREE.Vector2(-hx, -hy);
    ...
    const polygon = new EditablePolygon(vertices);
    polygon.minVertices = 3;
    polygon.isRectangular = false;
    if (polygon.edges?.[curvedEdgeIndex]) {
      polygon.edges[curvedEdgeIndex].isCurved = true;
      polygon.edges[curvedEdgeIndex].control = control;
    }
    return polygon;
  }

  applyStarterCustomGeometry(preset) {
    const outline = preset?.customOutline;
    if (!outline) return;

    if (outline.type === "rounded-rectangle-corner") {
      this.editablePolygon = this.createStarterRoundedCornerPolygon(outline);
      this.isCustomShape = true;
      this.baseShapeType = this.poolParams.shape || "rectangular";
    }
  }

  applyStarterTilePreference(preset) {
    const tileKey = preset?.tile;
    if (!tileKey || !this.pbrManager) return;

    this.pbrManager.currentTileKey = tileKey;
  ...
  async applyStarterPreset(preset) {
    ...
    this.selectedStep = null;
    this.selectedWall = null;
    this.applyStarterCustomGeometry(preset);
    this.applyStarterTilePreference(preset);
  ...
  async start(options = {}) {
    ...
    if (starterPreset?.params) {
      ...
      this.editablePolygon = null;
      this.wallRaiseBySourceEdge = {};
      this.applyStarterCustomGeometry(starterPreset);
    }
...
// PBR / Caustics integration should never hard-crash the app if a module fails
    ...
    try {
      ...
      this.applyStarterTilePreference(starterPreset);
    } catch (err) {
