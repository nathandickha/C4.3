  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Pool end walls at the far left/right can read as an extra wall depth in
        // section because their back half remains visible after clipping. Hide the
        // offending thin-X shell so the overlay can show only the cut face.
        if (root === this.poolGroup && intersectsFrontHalf && thinX && !poolBounds.isEmpty()) {
