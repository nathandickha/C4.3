  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Hide any spa/channel wall strips that sit completely outside the pool
        // plan extents in X. These are the extra end walls that appear when the
        // spa projects beyond the pool bounds in section view.
        const outsidePoolX = !poolBounds.isEmpty() && (
          box.max.x < (poolBounds.min.x - EPS) ||
          box.min.x > (poolBounds.max.x + EPS)
        );
        if ((obj.userData?.isSpaWall || obj.userData?.isSpaChannel) && outsidePoolX) {
          hideNow();
          return;
        }
