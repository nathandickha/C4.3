...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSectionBand = (x0, x3, includeWaterTint = true) => {
      ...
      // Do not draw the left end wall section cap here.
      // The user wants anything past the section cutting line hidden,
      // so leaving this cap visible creates the stray foreground strip.
      ...
      // Do not draw the right end wall section cap here.
      // The user wants anything past the section cutting line hidden,
      // so leaving this cap visible creates the stray foreground strip.

      // In section view the coping should only read directly above the
      // visible wall thickness, not extend further into the pool span.
      // This restores the missing coping over the end wall near the steps
      // while also preventing the coping behind the spa from overhanging
      // further than the wall below it.
      const copingLeftShape = new THREE.Shape([
        new THREE.Vector2(x0 - safeWallThickness, 0),
        new THREE.Vector2(x0, 0),
        new THREE.Vector2(x0, 0.05),
  ...
  _setSectionSideCopingHidden(enabled) {
    if (!this.poolGroup?.traverse) return;

    this.poolGroup.traverse((obj) => {
      if (!obj?.isMesh) return;
      if (!obj.userData?.isCoping) return;
      const side = String(obj.userData?.side || '').toLowerCase();
      if (side !== 'west' && side !== 'east') return;

      if (obj.userData.__sectionOriginalVisible == null) {
        obj.userData.__sectionOriginalVisible = obj.visible;
      }

      if (!enabled) {
        obj.visible = obj.userData.__sectionOriginalVisible;
        return;
      }
  ...
  _trimPoolSideWallAtSection(sectionY, enabled) {
    if (!this.poolGroup?.traverse) return;
    ...
    this.poolGroup.traverse((obj) => {
      ...
      const side = String(obj.userData?.side || '').toLowerCase();
      // Apply the section-side trimming symmetrically to both end walls.
      if (side !== 'west' && side !== 'east') return;
      ...
      // Keep the same half that worked in the previous west-wall patch.
  ...
  setSectionViewEnabled(enabled) {
    ...
    if (enabled) {
      ...
      this._trimPoolSideWallAtSection(sectionCutY, true);
      this._setSectionSideCopingHidden(true);
      ...
      this._trimPoolSideWallAtSection(0, false);
      this._setSectionSideCopingHidden(false);
