...
function createSingleRadiusCornerPolygon(length = 6, width = 4, radius = 2, corner = "entry-left") {
  const L = Math.max(0.5, Number(length) || 6);
  const W = Math.max(0.5, Number(width) || 4);
  const R = Math.max(0.05, Math.min(Number(radius) || 2, L * 0.5, W * 0.5));
  const minX = -L * 0.5;
  const maxX = L * 0.5;
  const minY = -W * 0.5;
  const maxY = W * 0.5;
  ...
  // Current project convention: steps start from the entry wall at minX.
  // "left" step position uses the minY side of that wall, so entry-left is minX/minY.
  if (corner === "entry-left") {
    pts.push(new THREE.Vector2(minX + R, minY));
    pts.push(new THREE.Vector2(maxX, minY));
    pts.push(new THREE.Vector2(maxX, maxY));
    pts.push(new THREE.Vector2(minX, maxY));
    pts.push(new THREE.Vector2(minX, minY + R));
    const cx = minX + R;
    const cy = minY + R;
    for (let i = 1; i <= segments; i += 1) {
      const a = Math.PI + (Math.PI * 0.5) * (i / segments);
      pts.push(new THREE.Vector2(cx + Math.cos(a) * R, cy + Math.sin(a) * R));
    }
  } else {
    pts.push(new THREE.Vector2(minX, minY));
    pts.push(new THREE.Vector2(maxX, minY));
    pts.push(new THREE.Vector2(maxX, maxY));
    pts.push(new THREE.Vector2(minX, maxY));
  }
  ...
  poly.userData = {
    ...(poly.userData || {}),
    starterFootprintType: "single-radius-corner",
    starterFootprintCorner: corner,
    starterFootprintRadius: R
  };
  return poly;
}

function createStarterPresetPolygon(starterPreset, params) {
  const footprint = starterPreset?.customFootprint;
  if (!footprint) return null;

  if (footprint.type === "single-radius-corner") {
    return createSingleRadiusCornerPolygon(
      params?.length,
      params?.width,
      footprint.radius,
      footprint.corner
...
function createStarterParamsPolygon(params) {
  const footprint = params?.customFootprint;
  if (!footprint) return null;
...
const STARTER_POOL_PRESETS = [
  {
    ...
    description: "6 x 4 m rectangle pool with a 2 m curved corner, left radius steps, bench seat and Arctic Blue tile.",
    preview: "rectangle",
    params: {
      shape: "rectangular",
      length: 6,
      ...
      stepExtension: 0.3,
      bench2Extension: 0.3,
      diagonalStepSize: 0.3,
      stepPosition: "left",
      stepShape: "radius",
      stepBenchMode: "bench",
      useShapeFloor: true,
      starterPresetId: "preset-1-curved-corner",
      customFootprint: { type: "single-radius-corner", corner: "entry-left", radius: 2 }
    },
    customFootprint: { type: "single-radius-corner", corner: "entry-left", radius: 2 },
    tileKey: "Arctic Blue",
    spa: null
...
// --------------------------------------------------------------
  ...
  getStepWidthSliderMax() {
    if (this.isCenteredCircularStepShape?.()) return Math.max(0.5, Number(this.poolParams?.width) || 5);
    ...
    return Math.max(0.5, Number(this.poolParams?.width) || 5);
  }
  ...
  previewStepWidthSlider(widthValue) {
    ...
    const getCenterY = (width, full = false) => {
      ...
      if (position === "left") return spanMinY + width * 0.5;
      if (position === "right") return spanMaxY - width * 0.5;
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      if (isDiagonal && selectedStepIndex !== 1) {
        ...
        this.previewStepWidthSlider(val);
        this._scheduleRebuildDebounced?.();
        return;
      }
  ...
  // --------------------------------------------------------------
  isPolygonShape() {
    return this.poolParams.shape === "freeform" || !!this.poolParams?.customFootprint || (!!this.editablePolygon && !!this.isCustomShape);
  }
  ...
  async rebuildPoolForCurrentShape() {
    ...
    // Starter preset 1 uses a rectangular base with a real custom footprint.
    // Rebuild the editable polygon from params here so the actual pool build path
    // cannot fall back to the plain rectangle if startup/preload state is stale.
    const starterParamsPolygon = createStarterParamsPolygon(this.poolParams);
    if (starterParamsPolygon) {
      this.editablePolygon = starterParamsPolygon;
...
updateGroundVoid(this.ground || this.scene.userData.ground, this.poolGroup, this.spa);
  ...
  async start(options = {}) {
    ...
    if (starterPreset?.params) {
      this.poolParams = {
        ...this.poolParams,
        ...starterPreset.params,
        customFootprint: starterPreset.params?.customFootprint || starterPreset.customFootprint || this.poolParams.customFootprint || null
      };
      this.baseShapeType = this.poolParams.shape;

      const starterPolygon = createStarterPresetPolygon(starterPreset, this.poolParams);
      if (starterPolygon) {
        this.editablePolygon = starterPolygon;
        this.isCustomShape = true;
      } else {
        this.isCustomShape = false;
        this.editablePolygon = null;
...
// PBR / Caustics integration should never hard-crash the app if a module fails
    ...
    try {
      ...
      const starterTileKey = starterPreset?.tileKey || starterPreset?.tile || starterPreset?.params?.tileKey;
      if (starterTileKey) this.pbrManager.currentTileKey = starterTileKey;
    } catch (err) {
...
setupPoolEditor() {
  ...
  _applyLivePreviewFromDirty() {
    ...
    const centeredCircularStepDirty = this.isCenteredCircularStepShape?.() && (
      this._live.dirty.has("stepWidth") ||
      this._live.dirty.has("stepCount")
    );
    if (centeredCircularStepDirty) {
  ...
  setupPoolSliders() {
    ...
    ids.forEach((id) => {
      ...
      slider.addEventListener("input", (e) => {
        ...
        if (id === "stepWidth") {
          if (this.isCenteredCircularStepShape?.()) {
            // Centre circular steps change topology, so use the accurate live
            // rebuild path instead of box-scaling the old meshes.
            this.poolGroup?.scale?.set?.(1, 1, 1);
            this._scheduleAccurateLiveRebuild?.();
          } else {
            this.previewStepWidthSlider(val);
