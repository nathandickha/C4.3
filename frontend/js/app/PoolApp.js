...
// -----------------------------
  ...
  _setSectionShellClip(root, plane) {
    if (!root) return;
    this._forEachSectionMaterial(root, (mat, obj) => {
      // Diagnostic build: apply section clipping to every mesh under pool/spa/channel roots.
  ...
  _runSectionDiagnostics(sectionCutY = null) {
    const cutY = Number.isFinite(sectionCutY) ? sectionCutY : this._getSectionCutY();
    const roots = [
      ['poolGroup', this.poolGroup],
      ['spa', this.spa],
      ['spaChannelGroup', this.ground?.userData?.spaChannelGroup]
    ].filter(([, root]) => !!root);
    ...
    roots.forEach(([rootName, root]) => {
      root.traverse((obj) => {
        ...
        const box = new THREE.Box3().setFromObject(obj);
        if (box.isEmpty()) return;
        const nearCut = !(box.max.y < (cutY - 0.35) || box.min.y > (cutY + 0.35));
        if (!nearCut) return;
        summary.push({
          root: rootName,
          name: obj.name || '',
          uuid: obj.uuid,
          side: obj.userData?.side || '',
          copingKey: obj.userData?.copingKey || '',
          ...
          isSpaSupport: !!obj.userData?.isSpaSupport,
    ...
    console.groupCollapsed?.('[Section Diagnostics]');
    console.log('cutY', cutY);
    console.log('snapSide', this.spa?.userData?.snapSide);
    console.log('snapVariant', this.spa?.userData?.snapVariant);
    console.log('isHalfwayInWall', this.spa?.userData?.isHalfwayInWall);
    console.log('channelEnabled', this.spa?.userData?.channelEnabled);
    console.log('poolBounds', poolBounds.isEmpty() ? null : poolBounds);
    console.log('spaBounds', spaBounds.isEmpty() ? null : spaBounds);
    console.table?.(summary);
    console.groupEnd?.();
  }

  _setDiagnosticSectionColors(enabled) {
    const applyToRoot = (root, colorHex) => {
      if (!root?.traverse) return;
      root.traverse((obj) => {
        if (!obj?.isMesh || !obj.material) return;
        const mats = Array.isArray(obj.material) ? obj.material : [obj.material];
        mats.forEach((mat) => {
          ...
          if (enabled) {
            if (mat.userData?.__diagPrevColor === undefined) {
              mat.userData = mat.userData || {};
              mat.userData.__diagPrevColor = mat.color.getHex();
            }
            mat.color.setHex(colorHex);
          } else if (mat.userData?.__diagPrevColor !== undefined) {
            mat.color.setHex(mat.userData.__diagPrevColor);
            delete mat.userData.__diagPrevColor;
          }
          mat.needsUpdate = true;
        });
      });
    };
    applyToRoot(this.poolGroup, 0x4aa3ff);
    applyToRoot(this.spa, 0x66cc88);
    applyToRoot(this.ground?.userData?.spaChannelGroup, 0xffaa44);
  }
  ...
  setSectionViewEnabled(enabled) {
    ...
    if (enabled) {
      ...
      this._setDiagnosticSectionColors(true);
      this._runSectionDiagnostics(sectionCutY);
      ...
      this._setDiagnosticSectionColors(false);
