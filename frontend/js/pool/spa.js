...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    // Diagnostic: the stubborn section wall has been isolated to the spa root.
    // When the spa is snapped into/past the pool wall, hide the outboard square
    // spa wall based on snapSide so we can verify that this is the exact mesh.
    ...
    const hideOutboardSpaWall = snapVariant !== "inner-flush";
    if (hideOutboardSpaWall) {
      if (snapSide === "left" && walls.left) walls.left.visible = false;
      if (snapSide === "right" && walls.right) walls.right.visible = false;
      if (snapSide === "front" && walls.front) walls.front.visible = false;
      if (snapSide === "back" && walls.back) walls.back.visible = false;
    }
