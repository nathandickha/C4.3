  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        const isSpaSideRoot = root !== this.poolGroup;
        ...
        // Hide the outboard spa-side wall/panel that remains in front of the
        // section line. The previous X-center test missed walls whose bounding
        // box still straddled the pool edge. Instead, use the spa snap side and
        // hide any spa-side shell that lies on the outboard side of the pool.
        if (isSpaSideRoot && intersectsFrontHalf && !poolBounds.isEmpty()) {
          if (snapSide === 'right' && box.max.x > (poolBounds.max.x + EPS)) {
            hideNow();
            return;
          }
          if (snapSide === 'left' && box.min.x < (poolBounds.min.x - EPS)) {
            hideNow();
            return;
          }
          if (snapSide === 'front' && box.max.y > (poolBounds.max.y + EPS)) {
            hideNow();
            return;
          }
          if (snapSide === 'back' && box.min.y < (poolBounds.min.y - EPS)) {
            hideNow();
            return;
          }
        }

        // Also hide the pool shell end wall on the same side as the attached
        // spa when that wall overlaps the spa footprint. This avoids leaving a
        // foreground pool end wall visible beside the spa in section view while
        // preserving the opposite end wall on a normal pool.
        if (root === this.poolGroup && intersectsFrontHalf && !spaBounds.isEmpty()) {
        ...
        // Standard pool/front-shell hide path.
        const isFrontBackStrip = thinY;
        if (!isFrontBackStrip) {
          restore();
          return;
        }
