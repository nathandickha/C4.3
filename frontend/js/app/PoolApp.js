  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Hide any spa-side shell meshes that sit completely outside the pool
        // plan extents in X. This catches not only tagged spa/channel walls,
        // but also coping and any end-wall body meshes living under the spa or
        // spa-channel roots.
        const outsidePoolX = !poolBounds.isEmpty() && (
          box.max.x < (poolBounds.min.x - EPS) ||
          box.min.x > (poolBounds.max.x + EPS)
        );
        const isSpaSideRoot = root !== this.poolGroup;
        if (isSpaSideRoot && outsidePoolX) {
          hideNow();
          return;
        }
