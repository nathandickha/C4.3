...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSpaChannelSectionCaps = () => {
      ...
      const addChannelFloorUndersideCap = (box) => {
        if (!box || box.isEmpty()) return;
        if (sectionY < box.min.y - 1e-4 || sectionY > box.max.y + 1e-4) return;
        if ((box.max.x - box.min.x) <= 1e-4) return;
        const bottomZ = box.min.z;
        const topZ = bottomZ + safeWallThickness;
        const shape = new THREE.Shape([
          new THREE.Vector2(box.min.x, topZ),
          new THREE.Vector2(box.max.x, topZ),
          new THREE.Vector2(box.max.x, bottomZ),
          new THREE.Vector2(box.min.x, bottomZ)
        ]);
        const cap = new THREE.Mesh(new THREE.ShapeGeometry(shape), floorMatBase.clone());
        cap.rotation.x = Math.PI * 0.5;
        cap.position.set(0, sectionY + 0.0055, 0);
        cap.renderOrder = 995;
        group.add(cap);
      };
      ...
      channelGroup.traverse((obj) => {
        ...
        const box = addProjectedCapFromMesh(obj, matBase, renderOrder, yOffset);
        if (part === 'floor') addChannelFloorUndersideCap(box);
      });
    };
  ...
  _trimPoolWestWallAtSection(sectionY, enabled) {
    ...
    this.poolGroup.traverse((obj) => {
      ...
      if (String(obj.userData?.side || '').toLowerCase() !== 'west') return;

      if (!obj.userData.__sectionOriginalGeometry) {
        obj.userData.__sectionOriginalGeometry = obj.geometry;
        obj.userData.__sectionOriginalPositionY = obj.position.y;
      }
      ...
      if (!enabled) {
        ...
        if (typeof obj.userData.__sectionOriginalPositionY === 'number') {
          obj.position.y = obj.userData.__sectionOriginalPositionY;
        }
        obj.visible = true;
        return;
      }
      ...
      if (sectionY <= box.min.y + EPS || sectionY >= box.max.y - EPS) {
        if (obj.userData.__sectionOriginalGeometry && obj.geometry !== obj.userData.__sectionOriginalGeometry) {
          obj.geometry.dispose?.();
          obj.geometry = obj.userData.__sectionOriginalGeometry;
        }
      ...
      const keepMinY = sectionY;
      const keepMaxY = box.max.y;
      ...
      const size = box.getSize(new THREE.Vector3());
      const center = box.getCenter(new THREE.Vector3());
      const trimmedGeo = new THREE.BoxGeometry(size.x, keptWidthY, size.z);

      if (obj.geometry !== obj.userData.__sectionOriginalGeometry) {
        obj.geometry.dispose?.();
      }
      obj.geometry = trimmedGeo;

      const newCenterWorldY = keepMinY + keptWidthY * 0.5;
      const parent = obj.parent;
      if (parent) {
        const localCenter = parent.worldToLocal(new THREE.Vector3(center.x, newCenterWorldY, center.z));
  ...
  _runRemainingStripDiagnostics(sectionY) {
    if (!this.poolGroup) return;

    const rows = [];
    const collect = (rootName, root) => {
      if (!root?.traverse) return;
      root.traverse((obj) => {
        ...
        const nearSection = sectionY >= (box.min.y - 0.08) && sectionY <= (box.max.y + 0.08);
        ...
        rows.push({
          ...
          name: obj.name || '',
          uuid: obj.uuid,
          side: String(obj.userData?.side || ''),
          copingKey: String(obj.userData?.copingKey || ''),
          isWall: !!obj.userData?.isWall,
          isCoping: !!obj.userData?.isCoping,
          isFloor: !!obj.userData?.isFloor,
          isStep: !!obj.userData?.isStep,
          isSpaWall: !!obj.userData?.isSpaWall,
          isSpaChannel: !!obj.userData?.isSpaChannel,
          xMin: +box.min.x.toFixed(3),
          xMax: +box.max.x.toFixed(3),
          yMin: +box.min.y.toFixed(3),
          yMax: +box.max.y.toFixed(3),
          zMin: +box.min.z.toFixed(3),
          zMax: +box.max.z.toFixed(3),
          sx: +size.x.toFixed(3),
          sy: +size.y.toFixed(3),
          sz: +size.z.toFixed(3),
          cx: +center.x.toFixed(3),
          cy: +center.y.toFixed(3),
          cz: +center.z.toFixed(3),
          material: obj.material?.type || ''
        });
      });
    };

    collect('poolGroup', this.poolGroup);
    collect('spa', this.spa);
    collect('spaChannelGroup', this.ground?.userData?.spaChannelGroup);

    console.groupCollapsed?.('[Remaining Strip Diagnostics]');
    console.log('sourceZip', "C43_west_wall_half_cut_patch.zip");
    console.log('sectionY', sectionY);
    console.log('spaSnapSide', this.spa?.userData?.snapSide);
    console.log('spaSnapVariant', this.spa?.userData?.snapVariant);
    console.table?.(rows);
    console.groupEnd?.();
  }
  ...
  setSectionViewEnabled(enabled) {
    ...
    if (enabled) {
      ...
      this._trimPoolWestWallAtSection(sectionCutY, true);
      this._setSectionOverlay(bounds, true, sectionCutY, wallThickness);
      this._runRemainingStripDiagnostics(sectionCutY);
      ...
      this._trimPoolWestWallAtSection(0, false);
