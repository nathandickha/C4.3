  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // side. This targets the outboard spa-wall case only and leaves the
        // normal internal-on-wall case alone.
        if (root === this.poolGroup && intersectsFrontHalf && !spaBounds.isEmpty() && !poolBounds.isEmpty()) {
          ...
          if ((snapSide === 'right' || snapSide === 'left') && overlapsSpaY) {
            const fullyOnSpaEnd = snapSide === 'right'
              ? box.min.x >= (poolBounds.max.x - edgeBand)
              : box.max.x <= (poolBounds.min.x + edgeBand);
            const shouldHide = fullyOnSpaEnd && (
              thinX ||
              (spaCutsPoolWall && overlapsSpaPlan)
            );
            if (shouldHide) {
              hideNow();
              return;
          ...
          if ((snapSide === 'front' || snapSide === 'back') && overlapsSpaX) {
            const fullyOnSpaEnd = snapSide === 'front'
              ? box.min.y >= (poolBounds.max.y - edgeBand)
              : box.max.y <= (poolBounds.min.y + edgeBand);
            const shouldHide = fullyOnSpaEnd && (
              thinY ||
              (spaCutsPoolWall && overlapsSpaPlan)
            );
