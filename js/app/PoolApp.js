  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // side. Prefer the explicit side/coping tags first, then fall back to
        // the older bbox heuristic only for untagged fragments.
        if (root === this.poolGroup && intersectsFrontHalf && !spaBounds.isEmpty() && !poolBounds.isEmpty()) {
          const objSideTag = String(obj.userData?.side || obj.userData?.copingKey || '').toLowerCase();
          if (spaCutsPoolWall && spaSideTag && objSideTag && objSideTag === spaSideTag) {
            hideNow();
            return;
          }
          ...
          if ((snapSide === 'right' || snapSide === 'left') && overlapsSpaY) {
            ...
            const shouldHide = !objSideTag && onSpaSide && (
              thinX ||
