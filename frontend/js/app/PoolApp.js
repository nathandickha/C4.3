...
// -----------------------------
  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Only remove the pool wall on the spa side that stays in front of the
        // section line when the spa creates the channel. Leave spa and channel
        // geometry alone so the channel remains visible.
        if (root === this.poolGroup && intersectsFrontHalf && !spaBounds.isEmpty() && !poolBounds.isEmpty()) {
          if ((snapSide === 'right' || snapSide === 'left') && thinX && overlapsSpaY) {
            const onSpaSide = snapSide === 'right'
              ? box.max.x > (poolBounds.max.x - 0.25)
              : box.min.x < (poolBounds.min.x + 0.25);
            if (onSpaSide) {
              hideNow();
              return;
            }
          }
          if ((snapSide === 'front' || snapSide === 'back') && thinY && overlapsSpaX) {
            const onSpaSide = snapSide === 'front'
              ? box.max.y > (poolBounds.max.y - 0.25)
              : box.min.y < (poolBounds.min.y + 0.25);
            if (onSpaSide) {
              hideNow();
              return;
            }
        ...
        // Do not blanket-hide the far pool end walls here. Doing that removes
        // the whole left/right end wall in section instead of trimming it at
        // the section plane. The standard shell clipping path below should keep
        // the wall cut at the section line, while the overlay supplies the cap
        // and the step caps stay visible.
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addPoolStepSectionCaps = () => {
      if (!this.poolGroup?.traverse) return;
      this.poolGroup.traverse((obj) => {
        if (!obj?.isMesh || obj.visible === false || !obj.userData?.isStep) return;
        addProjectedCapFromMesh(obj, wallCapMatBase, 999, 0.0085);
      });
    };

    const addPoolEndWallSectionCaps = () => {
      if (!this.poolGroup?.traverse) return;
      const poolBounds = new THREE.Box3();
      poolBounds.expandByObject(this.poolGroup);
      if (poolBounds.isEmpty()) return;
      const EPS = 1e-4;
      this.poolGroup.traverse((obj) => {
        if (!obj?.isMesh || obj.visible === false || !obj.userData?.isWall) return;
        obj.updateMatrixWorld?.(true);
        const box = new THREE.Box3().setFromObject(obj);
        if (box.isEmpty()) return;
        const size = box.getSize(new THREE.Vector3());
        const thinX = size.x <= Math.max(0.35, size.y * 0.4);
        const intersectsFrontHalf = box.min.y < (sectionY - EPS);
        if (!thinX || !intersectsFrontHalf) return;
        const nearLeftEnd = box.min.x < (poolBounds.min.x + 0.25);
        const nearRightEnd = box.max.x > (poolBounds.max.x - 0.25);
        if (!nearLeftEnd && !nearRightEnd) return;
    ...
    const shouldHideSpaSidePoolWallCap = (() => {
      ...
      const snapVariant = String(spa.userData?.snapVariant || 'inner-flush').toLowerCase();
      const hideOnSpaSide = snapVariant !== 'inner-flush';
      return {
        left: hideOnSpaSide && snapSide === 'left',
        right: hideOnSpaSide && snapSide === 'right'
      };
    })();
    ...
    addPoolStepSectionCaps();
    addPoolEndWallSectionCaps();
