...
// -----------------------------
  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    const poolSideKeyForSnapSide = (() => {
      if (snapSide === 'right') return 'east';
      if (snapSide === 'left') return 'west';
      if (snapSide === 'front') return 'south';
      if (snapSide === 'back') return 'north';
      return '';
    })();
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // When the spa is pushed into or beyond the pool wall, remove the pool
        // wall/coping mesh on that exact pool side in section view. This is
        // more reliable than pure bounding-box heuristics because the rectangle
        // pool walls/coping are tagged with side metadata.
        if (root === this.poolGroup && spaCutsPoolWall && poolSideKeyForSnapSide) {
          const objSide = String(obj.userData?.side || obj.userData?.copingKey || '').toLowerCase();
          if (objSide === poolSideKeyForSnapSide) {
            hideNow();
            return;
          }
        }

        // Fallback for any untagged spa-side shell fragment that still survives
        // on the outboard end when the spa is in/past the wall.
        if (root === this.poolGroup && intersectsFrontHalf && !spaBounds.isEmpty() && !poolBounds.isEmpty()) {
          ...
          if ((snapSide === 'right' || snapSide === 'left') && overlapsSpaY) {
            ...
            const shouldHide = spaCutsPoolWall && onSpaSide && (
              thinX ||
              (thinY && overlapsSpaPlan)
            );
            if (shouldHide) {
              hideNow();
              return;
