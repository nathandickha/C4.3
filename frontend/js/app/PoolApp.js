...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const shouldHideSpaSidePoolWallCap = (() => {
      const spa = this.spa;
      if (!spa) return { left: false, right: false, hideBothCopingCaps: false };
      ...
      if (!hideOnSpaSide) return { left: false, right: false, hideBothCopingCaps: false };
      ...
      if (!intersectsSection) return { left: false, right: false, hideBothCopingCaps: false };
      return {
        left: snapSide === 'left',
        right: snapSide === 'right',
        hideBothCopingCaps: true
      };
    })();
    ...
    const addSectionBand = (x0, x3, includeWaterTint = true) => {
      ...
      if (!shouldHideSpaSidePoolWallCap.hideBothCopingCaps) {
  ...
  _setPoolSideCopingHiddenAtSection(sectionY, enabled) {
    ...
    this.poolGroup.traverse((obj) => {
      ...
      if (String(obj.userData?.side || '').toLowerCase() !== targetSide) return;
      ...
      // Hide only the pool coping segment that belongs to the trimmed side wall.
      // Do not reshape it, because reshaping turned it into the long strip the user rejected.
      obj.visible = false;
    });
  }
  ...
  setSectionViewEnabled(enabled) {
    ...
    if (enabled) {
      ...
      this._setPoolSideCopingHiddenAtSection(sectionCutY, true);
      ...
      this._setPoolSideCopingHiddenAtSection(0, false);
