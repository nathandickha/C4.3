...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const shouldHideSpaSidePoolWallCap = (() => {
      ...
      return {
        ...
        // Remove the small coping end caps on both sides in spa-cut section view.
  ...
  _trimPoolSideWallAtSection(sectionY, enabled) {
    ...
    this.poolGroup.traverse((obj) => {
      ...
      const isCoping = !!obj.userData?.isCoping;
      if (!isWall && !isCoping) return;
      if (!targetSide) return;
      if (String(obj.userData?.side || obj.userData?.copingKey || '').toLowerCase() !== targetSide) return;
      ...
      // Keep the same back-half for the spa-side wall and its coping segment.
