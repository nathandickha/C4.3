...
// -----------------------------
  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    const poolSideKeyBySnapSide = {
      left: 'west',
      right: 'east',
      front: 'south',
      back: 'north'
    };
    const spaSidePoolKey = poolSideKeyBySnapSide[snapSide] || '';
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // In the outboard/in-wall spa states, remove the pool shell pieces on
        // the spa side that survive in front of the section cut. Side-tagged
        // pool walls/coping are the most reliable identifiers here.
        if (root === this.poolGroup && spaCutsPoolWall && intersectsFrontHalf && spaSidePoolKey) {
          const objSide = String(obj.userData?.side || obj.userData?.copingKey || '').toLowerCase();
          const isSpaSideTaggedPiece = objSide === spaSidePoolKey;
          if (isSpaSideTaggedPiece) {
            hideNow();
            return;
          }
        }

        // Fallback for any untagged pool shell fragment on the spa side that
        // overlaps the spa footprint and sits in the outboard edge band.
        if (root === this.poolGroup && spaCutsPoolWall && intersectsFrontHalf && !spaBounds.isEmpty() && !poolBounds.isEmpty()) {
          ...
          if ((snapSide === 'right' || snapSide === 'left') && overlapsSpaY) {
            const fullyOnSpaEnd = snapSide === 'right'
              ? box.min.x >= (poolBounds.max.x - 0.45)
              : box.max.x <= (poolBounds.min.x + 0.45);
            if (fullyOnSpaEnd && overlapsSpaPlan) {
              hideNow();
              return;
            }
          }
          if ((snapSide === 'front' || snapSide === 'back') && overlapsSpaX) {
            const fullyOnSpaEnd = snapSide === 'front'
              ? box.min.y >= (poolBounds.max.y - 0.45)
              : box.max.y <= (poolBounds.min.y + 0.45);
            if (fullyOnSpaEnd && overlapsSpaPlan) {
              hideNow();
              return;
            }
