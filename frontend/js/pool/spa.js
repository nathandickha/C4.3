...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    // When the spa is pushed into/past the pool wall on the pool's right side,
    // only suppress the single left-hand outboard spa wall fragment that reads
    // past the section line. Leave the other spa walls untouched.
    ...
    if (snapVariant !== "inner-flush" && snapSide === "right" && walls.left) {
