...
class PoolBuilder {
  ...
  build() {
    ...
    // Standard pools keep the older subdivided rectangular floor.
    // Custom starter/freeform pools can opt into a floor clipped to the actual perimeter
    // so rounded corners do not get visually filled back into a rectangle.
    const useShapeFloor = !!this.params?.useShapeFloor || !!this.params?.customFootprint;
    const floorGeo = useShapeFloor
    ...
    const floorCenterX = useShapeFloor ? 0 : cx;
    const floorCenterY = useShapeFloor ? 0 : cy;

    for (let i = 0; i < floorPos.count; i++) {
      const localX = floorPos.getX(i);
      const worldX = localX + floorCenterX;
    ...
    floor.position.set(floorCenterX, floorCenterY, 0);
    floor.receiveShadow = true;
    floor.userData.isFloor = true;
    floor.userData.type = "floor";
    floor.userData.usesShapeFloor = useShapeFloor;
    this.group.add(floor);

    
    this.group.userData.floorMesh = floor;
    this.group.userData.floorMeta = {
      cx: floor.position.x, cy: floor.position.y,
      bbLen, bbWid,
      axisStartWallX,
      axisEndX,
      originX,
