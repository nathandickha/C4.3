...
// -----------------------------
  ...
  _updateSpaDimensionHandles() {
    ...
    if (!this.spa || this.sectionViewEnabled) {
...
// -----------------------------
  ...
  _updateSectionDimensionHandles() {
    ...
    // Hide section handles in section view so screenshots/section presentations
    // stay clean with no on-screen handle widgets.
    const allow = false &&
      !!this.sectionViewEnabled &&
      !!this.poolGroup &&
      this.poolParams?.shape !== "freeform" &&
...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSpaSectionCaps = () => {
      ...
      const spaWallEntries = [];
      const spaSeatBoxes = [];

      const snapEdgeNormal = spa.userData?.snapEdge?.normal?.clone?.() || null;
      const spaCenterWorld = new THREE.Vector3();
      spa.getWorldPosition?.(spaCenterWorld);
        ...
        if (obj.userData?.isSpaWall) {
          if (obj.visible !== false) {
            ...
            if (!wallBox.isEmpty()) {
              ...
              const inwardScore = snapEdgeNormal
                ? wallCenter.clone().sub(spaCenterWorld).dot(new THREE.Vector3(snapEdgeNormal.x, snapEdgeNormal.y, 0))
                : 0;
              spaWallEntries.push({ obj, box: wallBox, inwardScore });
            }
          }
        }
      });

      const hideOuterEndWallCap = !!spa.userData?.snapVariant && spa.userData.snapVariant !== 'inner-flush';
      let outerEntry = null;
      if (hideOuterEndWallCap && spaWallEntries.length) {
        outerEntry = spaWallEntries.reduce((best, entry) => {
          if (!best) return entry;
          return entry.inwardScore < best.inwardScore ? entry : best;
        }, null);
      }
      spaWallEntries.forEach((entry) => {
        if (!entry?.obj) return;
        if (hideOuterEndWallCap && outerEntry && entry.obj === outerEntry.obj) return;
        addProjectedCapFromBounds(entry.box, wallCapMatBase, 999, 0.008);
      });
