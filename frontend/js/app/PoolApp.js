  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        const centerOutsidePoolX = !poolBounds.isEmpty() && (
          boxCenterX < (poolBounds.min.x - EPS) ||
          boxCenterX > (poolBounds.max.x + EPS)
        );
        const intersectsFrontHalf = box.min.y < (centerY - EPS);

        // Remove the foreground wall/panel that sits outboard of the pool when
        // the spa projects beyond the shell. This is the wall visible in front
        // of the section line; it should disappear with the rest of the front
        // shell even though it is an end wall, not a thin front/back strip.
        if (isSpaSideRoot && centerOutsidePoolX && intersectsFrontHalf) {
          hideNow();
          return;
        }
