...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    // Diagnostic v3: hide the outboard spa wall and the two short returns that
    // meet it when the spa is snapped into/past the pool wall.
    ...
    if (hideOutboardSpaWall) {
      if (snapSide === "left") {
        if (walls.left) walls.left.visible = false;
        if (walls.front) walls.front.visible = false;
        if (walls.back) walls.back.visible = false;
      }
      if (snapSide === "right") {
        if (walls.right) walls.right.visible = false;
        if (walls.front) walls.front.visible = false;
        if (walls.back) walls.back.visible = false;
      }
      if (snapSide === "front") {
