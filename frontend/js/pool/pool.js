...
class PoolBuilder {
  ...
  build() {
    ...
    const shouldClipFloorToPerimeter = !!this.polygon?.edges?.some?.((e) => !!e?.isCurved && !!e?.control) || this.polygon?.isRectangular === false;
    const floorGeo = shouldClipFloorToPerimeter
      ? new THREE.ShapeGeometry(shape, 256)
      : new THREE.PlaneGeometry(bbLen, bbWid, segmentsX, segmentsY);

    // PlaneGeometry is centered at the bbox centre and positioned below.
    // ShapeGeometry is already authored in pool/world XY coordinates, so it must
    // stay at 0,0 or curved-corner/custom floors appear as a rectangle again.
    const floorMeshOffsetX = shouldClipFloorToPerimeter ? 0 : cx;
    const floorMeshOffsetY = shouldClipFloorToPerimeter ? 0 : cy;
    ...
    if (stepCount > 0) {
      originX = axisStartWallX + getStepFootprintLength(this.params, stepCount, STEP_LENGTH);
    }

    for (let i = 0; i < floorPos.count; i++) {
      // local x -> world x
      const localX = floorPos.getX(i);
      const worldX = localX + floorMeshOffsetX;
    ...
    floor.position.set(floorMeshOffsetX, floorMeshOffsetY, 0);
...
/* =====================================================
    ...
    if (stepCount > 0) {
      ...
      const narrowLayout = getStepLayout(this.params, bb.min.y, bb.max.y);
      const fullStepLayout = getStepLayout(this.params, bb.min.y, bb.max.y, { fullWidth: true });
      ...
      const baseLayout = wantsBenchSeat && s === 1 ? fullStepLayout : narrowLayout;
        ...
        if (stepBenchMode === "stepsOnly") {
          const nestedFullWidth = Math.max(0.05, bb.max.y - bb.min.y);
          ...
          let nestedCenterY = (bb.min.y + bb.max.y) * 0.5;
          if (narrowLayout.position === "left") nestedCenterY = bb.min.y + nestedWidth * 0.5;
          if (narrowLayout.position === "right") nestedCenterY = bb.max.y - nestedWidth * 0.5;
        ...
        const x = isCenteredCircular
          ? (axisStartWallX + (stepBenchMode === "bench" && stepCount > 1 && s > 1 ? bench2Extension : 0) + stepRun * 0.5)
          : (stepBenchMode === "stepsOnly"
              ? axisStartWallX + stepRun * 0.5
              : (s <= 1
                  ? axisStartWallX + stepRun * 0.5
                  : axisStartWallX + bench2Extension + ((s - 2) * stepRun) + stepRun * 0.5));
