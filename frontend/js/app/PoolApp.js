  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Always suppress the left-most pool end wall fragment that sits in
        // front of the section line. This is the lingering vertical strip on
        // the far left of the section screenshot.
        if (root === this.poolGroup && intersectsFrontHalf && !poolBounds.isEmpty()) {
          const isPoolWallLike = !!(obj.userData?.isWall || obj.userData?.isCoping);
          const leftEndPad = 0.30;
          const onFarLeftEnd = box.max.x <= (poolBounds.min.x + leftEndPad);
          if (isPoolWallLike && onFarLeftEnd) {
            hideNow();
            return;
          }
        }
